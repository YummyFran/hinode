...
const ProjectHeader = ({title, description, projectId}) => {
  ...
  return (
    <div className="mb-4 flex gap-4 items-center">
        {
            isEditting.title ? 
                <input className="text-white font-bold" style={{ width: `${details.title.length * 8}px`}} type="text" value={details.title} onChange={e => setDetails(prev => ({...prev, title: e.target.value}))} onBlur={() => handleSave(prev => ({...prev, title: false}))} autoFocus/>
            :
        ...
        }
        {
            isEditting.description ?
                <input className="text-gray-100 text-sm" style={{ width: `${details.description.length * 8}px`}} type="text" value={details.description} onChange={e => setDetails(prev => ({...prev, description: e.target.value}))} onBlur={() => handleSave(prev => ({...prev, description: false}))} autoFocus/>
            :
